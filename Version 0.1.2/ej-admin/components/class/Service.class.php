...
class Service {
    ...
    public function set($id_adm) {
        ...
        if (!ValidationData::text($this->name)) {
            ...
            Dbcommand::insert('tb_servicos', 
                    array('SER_ID_ADM','SER_DATA','SER_LOG','SER_NOME','SER_SOBRE', 'SER_CHAVE'), 
                    array($this->id_adm, $this->date_in, $this->log, $this->name, $this->about, $this->key));
            return 6;
        }
    }
    ...
    public function update($id_adm, $url = 1){
        ...
        if (!ValidationData::text($this->name)){            
            ...
            Dbcommand::update('tb_servicos', 
                array('SER_ID_ADM' => $this->id_adm, 'SER_LOG' => $this->log, 'SER_NOME' => $this->name,'SER_SOBRE' => $this->about), 
                array('SER_ID' => $this->id));
            return 5;
        }
    }
