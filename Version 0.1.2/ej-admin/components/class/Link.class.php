...
class Link {
    ...
    public function set($id_adm) {
        ...
        if (!ValidationData::text($this->name) || !ValidationData::text($this->url)) {
            ...
            Dbcommand::insert('tb_links', 
                    array('LINK_ID_ADM','LINK_DATA','LINK_LOG','LINK_NOME','LINK_SOBRE', 'LINK_CHAVE', 'LINK_URL'), 
                    array($this->id_adm, $this->date_in, $this->log, $this->name, $this->about, $this->key, $this->url));
            return 6;
        }
    }
    ...
    public function update($id_adm, $url = 1){
        ...
        if (!ValidationData::text($this->name) || !ValidationData::text($this->url)){            
            ...
            Dbcommand::update('tb_links', 
                array('LINK_ID_ADM' => $this->id_adm, 'LINK_LOG' => $this->log, 'LINK_NOME' => $this->name,'LINK_SOBRE' => $this->about, 'LINK_URL' => $this->url), 
